...
def run_task(task: TaskLog, event, args: str = ""):
    err_file = open(Path(settings.LOG_DIR, "task_runner.err"), "a+", encoding=settings.ENCODING)
    cmd = [
...
def prepare_ansible_config(job_id: int, action: Action, sub_action: SubAction):
    ...
    with open(
        Path(settings.RUN_DIR, f"{job_id}", "ansible.cfg"), "w", encoding=settings.ENCODING
    ) as config_file:
