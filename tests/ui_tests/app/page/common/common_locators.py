...
class ObjectPageMenuLocators:
    ...
    service_components_tab = Locator(
        By.CSS_SELECTOR, "a[adcm_test='tab_component']", "Tab service components"
    )
    config_tab = Locator(By.CSS_SELECTOR, "a[adcm_test='tab_config']", "Tab config")
    group_config_tab = Locator(
        By.CSS_SELECTOR, "a[adcm_test='tab_group_config']", "Tab group config"
    )
