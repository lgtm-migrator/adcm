...
class TestClusterListPage:
    ...
    def test_run_upgrade_on_cluster_list_page(self, sdk_client_fs, app_fs):
        ...
        cluster_page.run_upgrade_in_cluster_row(
            row=row_with_upgrade, upgrade_name=params["upgrade"]
        )
...
class TestClusterMainPage:
    ...
    def test_check_cluster_main_page_open_by_toolbar(self, app_fs, create_community_cluster):
        ...
        cluster_import_page = ClusterImportPage(
            app_fs.driver, app_fs.adcm.url, create_community_cluster.id
        ).open()
    ...
    def test_check_cluster_run_action_on_cluster_page_by_toolbar(
        self, app_fs, create_community_cluster
    ):
...
class TestClusterServicePage:
    ...
    def test_concerns_from_from_service_list_page(self, sdk_client_fs: ADCMClient, app_fs):
        ...
        cluster_service_page = ClusterServicesPage(
            app_fs.driver, app_fs.adcm.url, cluster.id
        ).open()
    ...
    @pytest.mark.smoke()
    @pytest.mark.include_firefox()
    def test_check_actions_from_service_list_page(
        self, app_fs, create_community_cluster_with_service
    ):
    ...
    def test_check_service_list_page_open_service_config(
        self, app_fs, create_community_cluster_with_service
    ):
        ...
        service_conf_page = ServiceConfigPage(
    ...
    def test_delete_service_on_service_list_page(
        self, app_fs, create_community_cluster_with_service
    ):
        ...
        with allure.step("Check that after deleting service row there are no rows"):
            assert (
                len(cluster_service_page.table.get_all_rows(timeout=2)) == 0
            ), "There should not be any rows"
...
class TestClusterHostPage:
    ...
    @pytest.mark.smoke()
    @pytest.mark.include_firefox()
    def test_required_fields_from_cluster_host_page(
        self, app_fs, create_community_cluster_with_service
    ):
    ...
    def test_create_host_from_cluster_host_page(
        self, app_fs, create_community_cluster_with_service
    ):
    ...
    def test_create_host_error_from_cluster_host_page(
        self, app_fs, create_community_cluster_with_service
    ):
    ...
    def test_open_host_concern_from_cluster_host_page(
        self, app_fs, create_community_cluster_with_host
    ):
    ...
    def test_check_delete_host_from_cluster_host_page(
        self, app_fs, create_community_cluster_with_host
    ):
    ...
    def test_open_host_config_from_cluster_host_page(
        self, app_fs, create_community_cluster_with_host
    ):
    ...
    @pytest.mark.smoke()
    def test_maintenance_mode_from_cluster_host_page(
        self, app_fs, create_community_cluster_with_host
    ):
...
class TestClusterComponentsPage:
    ...
    def test_check_cluster_components_page_open_service_page(
        self, app_fs, create_community_cluster
    ):
        ...
        service_page = ClusterServicesPage(
            app_fs.driver, app_fs.adcm.url, create_community_cluster.id
        )
    ...
    def test_check_cluster_components_page_create_host(
        self, app_fs, bundle_archive, create_community_cluster
    ):
    ...
    def test_check_cluster_components_page_create_components(
        self, app_fs, sdk_client_fs, create_host, bundle_name
    ):
...
class TestClusterConfigPage:
    ...
    def test_save_custom_config_on_cluster_config_page(
        self, app_fs, create_cluster_with_all_config_fields
    ):
    ...
    def test_field_validation_on_cluster_config_page_with_default_value(
    ...
    def test_number_validation_on_cluster_config_page(
        ...
        with allure.step('Check that save button is active'):
            ...
            assert (
                not cluster_config_page.config.is_save_btn_disabled()
        ...
        with allure.step('Check that save button is disabled'):
            assert (
                cluster_config_page.config.is_save_btn_disabled()
            ), 'Save button should be disabled'
    ...
    def test_number_in_range_values_on_cluster_config_page(
        self, sdk_client_fs: ADCMClient, value, app_fs, number_type
    ):
        """Test save button is active for valid number values"""

        path = (
            get_data_dir(__file__)
            + f"/bundles_for_numbers_tests/{number_type}-positive_and_negative"
        )
        _, cluster_config_page = prepare_cluster_and_open_config_page(sdk_client_fs, path, app_fs)
        cluster_config_page.config.type_in_field_with_few_inputs(
...
class TestClusterGroupConfigPage:
    ...
    def test_open_by_tab_group_config_host_cluster_page(self, app_fs, create_community_cluster):
        ...
        cluster_group_config = create_community_cluster.group_config_create(
    ...
    def test_create_group_config_cluster(self, app_fs, create_community_cluster):
        ...
        with cluster_group_conf_page.group_config.wait_rows_change(expected_rows_amount=1):
            cluster_group_conf_page.group_config.create_group(
    ...
    def test_check_pagination_on_group_config_component_page(
        ...
        group_conf_page = ClusterGroupConfigPage(
            app_fs.driver, app_fs.adcm.url, create_community_cluster.id
        ).open()
...
class TestClusterStatusPage:
    ...
    def test_status_on_cluster_status_page(
        ...
        with allure.step("Check negative status on service"):
            ...
            cluster_status_page.driver.refresh()
            cluster_status_page.compare_current_and_expected_state(
                self.host_and_component_negative_status
            )
        with allure.step("Check collapse button"):
            ...
            assert (
                len(cluster_status_page.get_all_rows()) == 1
    ...
    def test_service_passive_status_on_cluster_status_page(
        ...
        with allure.step("Check negative status on service"):
            ...
            service_status_page.driver.refresh()
            service_status_page.compare_current_and_expected_state(
                self.host_and_component_negative_status
            )
...
class TestClusterImportPage:
    ...
    def test_cluster_import_from_cluster_import_page(
...
class TestClusterRenaming:
    ...
    def _test_correct_name_can_be_set(self, cluster: Cluster, page: ClusterListPage) -> None:
        ...
        with allure.step("Check name of cluster in table"):
            ...
            ), f"Cluster name on backend is incorrect, expected: {new_name}"

    def _test_an_error_is_shown_on_incorrect_char_in_name(
        self, cluster: Cluster, page: ClusterListPage
    ) -> None:
        dummy_name = "clUster"
        incorrect_names = (
        ...
        for cluster_name in incorrect_names:
            with allure.step(
                f"Check if printing cluster name '{cluster_name}' triggers a warning message"
            ):
                dialog.set_new_name_in_rename_dialog(dummy_name)
                dialog.set_new_name_in_rename_dialog(cluster_name)
                assert (
                    dialog.is_dialog_error_message_visible()
                ), "Error about incorrect name should be visible"
                assert (
    ...
    def _test_an_error_is_not_shown_on_correct_char_in_name(
        self, cluster: Cluster, page: ClusterListPage
    ) -> None:
        dummy_name = "clUster"
        correct_names = (
            *[
                f"{dummy_name[0]}{char}{dummy_name[1:]}"
                for char in (*self.DISALLOWED_AT_START_END, "9")
            ],
            f"9{dummy_name}",
        )
        ...
        for cluster_name in correct_names:
            with allure.step(f"Check if printing cluster name '{cluster_name}' shows no error"):
                ...
                dialog.set_new_name_in_rename_dialog(cluster_name)
                assert (
                    not dialog.is_dialog_error_message_visible()
                ), "Error about correct name should not be shown"
                dialog.click_save_on_rename_dialog()
                name_in_row = page.get_cluster_info_from_row(0)["name"]
                assert (
                    name_in_row == cluster_name
                ), f"Incorrect cluster name, expected: {cluster_name}"
